...
Dropzone.options.dpzRemoveAllThumb = {
	...
	init: function () {
		this.on('addedfile', function (file) {
			...
			// 	toastr.warning('Please Fill General Form', 'Warnning', { "closeButton": true });
			...
			$.ajax({
				...
				success: function (message) {
					if (message == 'success') {
						...
						toastr.error('Delete Success', 'Error', { "closeButton": true });
					}
				}
			});
		});
...
function copypriceset(copy_index) {
	...
	else if (category_flag == 3) {
		title = 'Activity Type';
	}
	else if (category_flag == 4) {
		title = 'Guide Type';
	}
	else if (category_flag == 5) {
		title = 'Type';
	}
